...
############################
# Benchmarking Test
@testset "OCP Benchmark Tests" begin
v0 = -2
h0 = 10
pts = 10000
...
#    for i in 1:length(x1); setvalue(x1[i], 0.0); setvalue(x2[i], 0.0);  end
    ...
    t_opt_ave[num] = mean(t_solve[.!isnan.(t_solve)])[1]
    h_error_ave[num] = mean(h_error[.!isnan.(h_error)])[1]
    v_error_ave[num] = mean(v_error[.!isnan.(v_error)])[1]
    u_error_ave[num] = mean(u_error[.!isnan.(u_error)])[1]
    max_error_ave[num] = mean(max_error[.!isnan.(max_error)])[1]
  end
